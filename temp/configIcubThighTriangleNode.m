...
meshDesign.diameter  = 0.111;
meshDesign.length    = 0.180;
meshDesign.m         = 8;
meshDesign.baseIndex = 1;

meshDesign.linkDimension    = meshDesign.diameter * sin(pi/meshDesign.m); %to check sin->tan
[~,meshDesign.angleV,meshDesign.unitLength] = CellMBodyCreation.getYoshimuraBucklingDimensions(meshDesign.diameter,meshDesign.m);

meshDesign.n = round(meshDesign.length/meshDesign.unitLength);

meshDesign.fixedLinksIndexes = [meshDesign.baseIndex];
meshDesign.tform_0_bBase = getTformGivenPosRotm([0 0 meshDesign.diameter/2]',createRotationMatrix(@ry,meshDesign.angleV/2,@rz,pi-pi/6));
...
cellCreator.assignLinkProperty('name','stlFaceColor','value','none','indexes',1)
